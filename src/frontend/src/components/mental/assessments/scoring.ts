...
export function computeAssessmentResult(
  ...
  const burnoutSum = Object.values(burnoutAnswers).reduce(
    (sum: number, val) => {
      const numVal = Array.isArray(val) ? val[0] : val;
      return sum + (typeof numVal === "number" ? numVal : 0);
    },
  ...
  const bigFive = [
    ...
    6 -
      (typeof bigFiveAnswers.bigfive_2 === "number"
        ? bigFiveAnswers.bigfive_2
        : 3), // Reverse scored
    typeof bigFiveAnswers.bigfive_3 === "number" ? bigFiveAnswers.bigfive_3 : 3,
    6 -
      (typeof bigFiveAnswers.bigfive_4 === "number"
        ? bigFiveAnswers.bigfive_4
        : 3), // Reverse scored
  ...
  const copingMode =
    copingValues.sort(
      (a, b) =>
        copingValues.filter((v) => v === a).length -
        copingValues.filter((v) => v === b).length,
  ...
  const stressTriggers = triggerArray
    .map((idx) => triggerLabels[idx - 1])
    .filter(Boolean);
