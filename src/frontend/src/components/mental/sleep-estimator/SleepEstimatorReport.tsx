...
export default function SleepEstimatorReport({
  result,
  onBack,
  input,
  isSavedRun = false,
}: SleepEstimatorReportProps) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex gap-2">
          {canSave && input && (
            <Button
              variant="outline"
              onClick={handleSave}
              disabled={saveSleepRun.isPending}
            >
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>Data stored:</strong> This tool uses behavioral phone-usage
          metadata and derived metrics only. No content is captured.
        </AlertDescription>
      </Alert>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            Sleep Analysis Summary (Last {result.daysAnalyzed} Days)
          </CardTitle>
          <CardDescription>
            Pattern-based sleep cycle estimation
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-2xl font-bold">
                {result.sleepConsistencyScore}/100
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="space-y-1">
              ...
              <Badge
                variant={
                  result.circadianStability === "Stable" ? "default" : "outline"
                }
              >
          ...
          <div className="space-y-1">
            <p className="text-sm text-muted-foreground">
              Night Disruption Frequency
            </p>
            <p className="font-medium">
              {result.nightDisruptionFrequency} checks/night
            </p>
          </div>
        </CardContent>
      </Card>
      ...
      {result.riskIndicators.length > 0 && (
        <Card className="border-amber-500/50">
          <CardHeader>
            ...
            <CardDescription>
              Patterns that may affect sleep quality
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ul className="space-y-2">
              {result.riskIndicators.map((risk) => (
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <ul className="list-disc list-inside space-y-1 text-sm text-muted-foreground">
                {result.advancedMetrics.remCycleTiming.map((cycle) => (
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {result.advancedMetrics.idealBedtimeWindow}
              </p>
            </div>
            <Separator />
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {result.advancedMetrics.recoveryProjection}
              </p>
            </div>
          </CardContent>
        </Card>
