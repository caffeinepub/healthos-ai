...
const _SUSTAINED_ACTIVITY_MINUTES = 3;
...
export function analyzeSleepPatterns(
  ...
  // Analyze each day
  const dailyAnalyses: DailyAnalysis[] = input.days.map((day) =>
    analyzeSingleDay(day),
  );
...
function analyzeSingleDay(day: DailySleepInput): DailyAnalysis {
  ...
  if (day.nightChecks && day.nightChecks.length > 0) {
    ...
    if (significantChecks.length > 0) {
      const lastCheckMins = getMinutesFromTime(
        significantChecks[significantChecks.length - 1].time,
      );
      if (
        lastCheckMins > lastActivityMins ||
        (lastCheckMins < 6 * 60 && lastActivityMins > 18 * 60)
      ) {
  ...
  const sleepOnsetMins =
    (lastActivityMins + INACTIVITY_THRESHOLD_MINUTES) % (24 * 60);
...
function computeAverageSleepOnset(analyses: DailyAnalysis[]): string {
  ...
  const avgMins =
    onsetMinutes.reduce((sum, m) => sum + m, 0) / onsetMinutes.length;
...
function computeConsistencyScore(analyses: DailyAnalysis[]): number {
  ...
  const onsetMinutes = analyses.map(
    (a) => a.sleepOnset.getHours() * 60 + a.sleepOnset.getMinutes(),
  );
  const onsetMean =
    onsetMinutes.reduce((sum, m) => sum + m, 0) / onsetMinutes.length;
  const onsetVariance =
    onsetMinutes.reduce((sum, m) => sum + (m - onsetMean) ** 2, 0) /
    onsetMinutes.length;
  const onsetStdDev = Math.sqrt(onsetVariance);

  // Wake time variance
  const wakeMinutes = analyses.map(
    (a) => a.wakeTime.getHours() * 60 + a.wakeTime.getMinutes(),
  );
  const wakeMean =
    wakeMinutes.reduce((sum, m) => sum + m, 0) / wakeMinutes.length;
  const wakeVariance =
    wakeMinutes.reduce((sum, m) => sum + (m - wakeMean) ** 2, 0) /
    wakeMinutes.length;
  ...
  const avgDeviation =
    durationDeviations.reduce((sum, d) => sum + d, 0) /
...
function computeCircadianStability(analyses: DailyAnalysis[]): string {
  ...
  const variance =
    midpoints.reduce((sum, m) => sum + (m - mean) ** 2, 0) / midpoints.length;
...
function computeNightDisruptionFrequency(days: DailySleepInput[]): number {
  const totalChecks = days.reduce(
    (sum, day) => sum + (day.nightChecks?.length || 0),
    0,
  );
...
function identifyRiskIndicators(
  ...
  for (const analysis of analyses) {
    if (analysis.duration < 6) {
      ...
      maxConsecutiveShortSleep = Math.max(
        maxConsecutiveShortSleep,
        consecutiveShortSleep,
      );
    } else {
  ...
  if (maxConsecutiveShortSleep >= 3) {
    risks.push(
      "Pattern suggests insufficient sleep duration (< 6 hours) for multiple consecutive days",
    );
  }
  ...
  if (wakeStdDev > 90) {
    risks.push(
      "Data indicates high wake time variability (> 90 minutes), which may affect circadian rhythm",
    );
  }
  ...
  if (nightChecks2to4AM.length >= days.length * 0.3) {
    // 30% or more nights
    risks.push(
      "Pattern suggests frequent phone use during deep sleep window (2-4 AM)",
    );
  }
  ...
  if (lateOnsets.length >= analyses.length * 0.4) {
    // 40% or more nights
...
function computeAdvancedMetrics(
  ...
  const idealBedtimeMins =
    (wakeTargetMins -
      idealSleepDurationMins -
