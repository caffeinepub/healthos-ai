...
export function buildMLExport(
  ...
  const derivedFeatures: MLFeatureVector[] = input.days.map((day, _index) => {
    // Last significant interaction (in minutes from midnight)
    const lastSignificantInteractionMinutes = getMinutesFromTime(
      day.lastActivity,
    );

    // First sustained interaction (in minutes from midnight)
    const firstSustainedInteractionMinutes = getMinutesFromTime(
      day.firstActivity,
    );
    ...
    const nightActiveMinutes =
      day.nightChecks?.reduce((sum, check) => {
    ...
    const inactivityBlockDuration =
      day.inactivityBlocks?.reduce((max, block) => {
...
export function downloadMLExport(
  data: MLExportData,
  filename = "sleep-analysis-ml-export.json",
): void {
