...
import type { DailySleepInput, SleepAnalysisInput } from "./types";
...
export function parseStructuredInput(
  ...
  if (!isValidTimeZone(timeZone)) {
    errors.push(
      `Invalid IANA time zone: ${timeZone}. Please use a valid timezone like "America/New_York" or "Europe/London".`,
    );
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.match(/^Last activity:/i)) {
      ...
      if (isValidTimeFormat(time)) {
        ...
        errors.push(
          `Day ${dayIndex}: Invalid time format for last activity: "${time}". Expected HH:MM format (e.g., "23:45").`,
        );
      }
      ...
      if (isValidTimeFormat(time)) {
        ...
        errors.push(
          `Day ${dayIndex}: Invalid time format for first activity: "${time}". Expected HH:MM format (e.g., "07:30").`,
        );
      }
      ...
      if (match) {
        ...
        errors.push(
          `Day ${dayIndex}: Invalid screen time format: "${content}". Expected format like "5h 30m" or "5h 30".`,
        );
      }
    }
  }
  ...
  if (days.length < 7) {
    errors.push(
      `Insufficient data: ${days.length} days provided, minimum 7 required for analysis.`,
    );
  } else if (days.length > 30) {
    errors.push(
      `Too much data: ${days.length} days provided, maximum 30 allowed.`,
    );
  }
...
function validateDay(
  ...
  if (!day.lastActivity) {
    errors.push(
      `Day ${dayIndex} (${date}): Missing required field "Last activity".`,
    );
    valid = false;
  }
  ...
  if (!day.firstActivity) {
    errors.push(
      `Day ${dayIndex} (${date}): Missing required field "First activity".`,
    );
    valid = false;
  }
  ...
  if (day.totalScreenTimeMinutes === undefined) {
    errors.push(
      `Day ${dayIndex} (${date}): Missing required field "Total screen time".`,
    );
    valid = false;
  }
...
export function parseJSONInput(input: string, timeZone: string): ParseResult {
  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (!day.lastActivity) {
        errors.push(
          `Day ${dayIndex} (${day.date}): Missing required field "lastActivity".`,
        );
        continue;
      }
      ...
      if (!day.firstActivity) {
        errors.push(
          `Day ${dayIndex} (${day.date}): Missing required field "firstActivity".`,
        );
        continue;
      }
      ...
      if (day.totalScreenTimeMinutes === undefined) {
        errors.push(
          `Day ${dayIndex} (${day.date}): Missing required field "totalScreenTimeMinutes".`,
        );
        continue;
      }
      ...
      if (!isValidTimeFormat(day.lastActivity)) {
        errors.push(
          `Day ${dayIndex} (${day.date}): Invalid time format for lastActivity: "${day.lastActivity}". Expected HH:MM format.`,
        );
        continue;
      }
      ...
      if (!isValidTimeFormat(day.firstActivity)) {
        errors.push(
          `Day ${dayIndex} (${day.date}): Invalid time format for firstActivity: "${day.firstActivity}". Expected HH:MM format.`,
        );
        continue;
      }
