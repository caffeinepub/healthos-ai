...
import type { NormalizedSleepInput, SleepEstimatorRun } from "../../../backend";
import { getMinutesFromTime } from "./timeUtils";
import type { SleepAnalysisInput, SleepAnalysisOutput } from "./types";
...
export function convertToBackendRun(
  ...
  const normalizedDailyInputs: NormalizedSleepInput[] = input.days.map(
    (day) => {
  ...
  const summaryMetrics = {
    ...
    circadianRhythmScore:
      output.circadianStability === "Stable"
        ? 0.9
        : output.circadianStability === "Moderate"
          ? 0.6
          : 0.3,
    sleepTrend:
      output.sleepDebt > 0
  ...
  const riskIndicators = {
    ...
    riskLevel: BigInt(
      output.riskIndicators.length > 2
        ? 7
...
export function convertFromBackendRun(run: SleepEstimatorRun): UIReportModel {
  ...
  const output: SleepAnalysisOutput = {
    ...
    averageSleepOnset: formatMinutesToTime(
      run.summaryMetrics.sleepLatency * 60,
    ), // Approximation
    averageWakeTime: "07:30", // Placeholder - not stored in backend
    averageDuration: run.summaryMetrics.averageSleepDuration,
    sleepConsistencyScore: Math.round(
      run.summaryMetrics.sleepConsistencyScore * 100,
    ),
    circadianStability:
      run.summaryMetrics.circadianRhythmScore > 0.8
        ? "Stable"
        : run.summaryMetrics.circadianRhythmScore > 0.5
          ? "Moderate"
          : "Unstable",
